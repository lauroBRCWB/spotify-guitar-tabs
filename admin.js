// Admin backoffice for managing songs
// Songs are stored in memory and can be exported to update the public app
...
class AdminApp {
  ...
  updateStats() {
    const cifraCount = this.songs.filter(s => s.cifraClubUrl).length;
    const ugCount = this.songs.filter(s => s.ultimateGuitarUrl).length;
    
    document.getElementById('totalSongs').textContent = this.songs.length;
    document.getElementById('cifraCount').textContent = cifraCount;
    document.getElementById('ugCount').textContent = ugCount;
  }
  ...
  setupEventListeners() {
    ...
    document.getElementById('searchSongs').addEventListener('input', () => this.renderSongList());
    document.getElementById('copyBtn').addEventListener('click', () => this.copyToClipboard());
    document.getElementById('downloadBtn').addEventListener('click', () => this.downloadAppJs());
  }
    ...
    tbody.innerHTML = filtered.map((song, index) => {
      ...
      return `
        <tr>
          ...
          <td>${links.join('')}</td>
          <td><button class="delete-btn" onclick="adminApp.deleteSong(${index})">Delete</button></td>
        </tr>
      `;
    }).join('');
  }
  ...
  updateExport() {
    const json = JSON.stringify(this.songs, null, 2);
    document.getElementById('exportArea').value = `const ALL_SONGS = ${json};`;
  }
  ...
  downloadAppJs() {
    const json = JSON.stringify(this.songs, null, 2);
    const content = `const ALL_SONGS = ${json};
...
class SongApp {
  ...
  populateFilters() {
    ...
    styles.forEach(style => {
      ...
      document.getElementById('styleFilter').appendChild(option);
    });
    ...
    artists.forEach(artist => {
      ...
      document.getElementById('artistFilter').appendChild(option);
    });
  }
  
  setupEventListeners() {
    document.getElementById('searchInput').addEventListener('input', () => this.filter());
    document.getElementById('styleFilter').addEventListener('change', () => this.filter());
    document.getElementById('artistFilter').addEventListener('change', () => this.filter());
    document.getElementById('sourceFilter').addEventListener('change', () => this.filter());
  }
  ...
  render() {
    document.getElementById('resultText').textContent = this.filteredSongs.length + ' songs';
    ...
    document.getElementById('songsTableBody').innerHTML = this.filteredSongs.map(song => {
      const links = [];
